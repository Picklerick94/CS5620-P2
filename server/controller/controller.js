...
const controller = () => {
  ...
  exported.findOne = async (req, res) => {
    if (!req.body) {
      res.status(400).send({ message: "student can not be empty" });
      return;
    }
  ...
  // User loggin
  exported.authenticate = async (req, res) => {
    const user = req.body;
